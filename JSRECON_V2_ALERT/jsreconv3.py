    ...
    for domain in sorted(domain_finding_map.keys()):
        ...
        for f in items:
            ...
            finding_rows += (
                ...
                f'<td class="td-url"><div class="host-chip">{f["host"]}</div>' +
                f'<a href="{f["url"]}" target="_blank" class="url-link js-url-link" title="{f["url"]}">{js_short}</a></td>' +
...
def send_telegram(cfg: dict, target_name: str, report_stats: dict,
    ...
    """Send summary message + report file to Telegram."""
    token   = cfg.get("telegram_bot_token", "").strip()
    chat_id = cfg.get("telegram_chat_id", "").strip()

    if not token or not chat_id:
        warn("Telegram: credentials missing in jsrecon.env")
        warn("  Set TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID")
        return
    if "your_bot_token_here" in token or "your_chat_id_here" in chat_id:
        warn("Telegram: placeholder credentials detected in jsrecon.env — skipping")
        return

    info(f"Sending Telegram notification to chat_id {chat_id}...")
    ...
    msg_lines = [
        "⚡ *JSRecon Cyborg V1 — by Rahul Masal*",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        f"🎯 *Target:* `{target_name}`",
        f"🕐 *Date:* {datetime.now().strftime('%Y-%m-%d %H:%M UTC')}",
        "",
        f"{overall_emoji} *Overall Severity:* `{overall_sev}` (CVSS {overall_score:.1f})",
        "",
        "📊 *Finding Counts:*",
        ...
        f"  • JS w/ Secrets  : `{len(secrets_data)}`",
        f"  • API Endpoints  : `{report_stats.get('endpoints', 0)}`",
        f"  • Sensitive Paths: `{report_stats.get('sensitive', 0)}`",
        "",
        "📎 *Full report attached below* ↓",
    ]
    msg = "\n".join(msg_lines)

    # Telegram MarkdownV2 mode escapes — use HTML mode instead for reliability
    msg_html_lines = [
        "⚡ <b>JSRecon Cyborg V1 — by Rahul Masal</b>",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        f"🎯 <b>Target:</b> <code>{target_name}</code>",
        f"🕐 <b>Date:</b> {datetime.now().strftime('%Y-%m-%d %H:%M')}",
        "",
        f"{overall_emoji} <b>Overall Severity:</b> <code>{overall_sev}</code> (CVSS {overall_score:.1f})",
        "",
        "<b>📊 Finding Counts:</b>",
    ...
    # ── Send text message (use HTML parse mode — more reliable) ──────────
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=30) as resp:
            ...
            if resp_body.get("ok"):
                ...
                warn(f"Telegram sendMessage failed: {resp_body.get('description','')}")
    except urllib.error.HTTPError as e:
        body = e.read().decode("utf-8", errors="replace")
        warn(f"Telegram sendMessage error {e.code}: {body[:300]}")
        if e.code == 401:
            warn("Telegram: 401 Unauthorized — bot token is invalid or revoked.")
        elif e.code == 400:
            warn("Telegram: 400 Bad Request — check chat_id is correct.")
        return
    except Exception as e:
        warn(f"Telegram sendMessage exception: {e}")
        return
    ...
    if attach_path and os.path.exists(attach_path):
        caption = f"JSRecon Report — {target_name} | {overall_sev} (CVSS {overall_score:.1f})"
        _tg_send_document(token, chat_id, attach_path, caption)
    else:
        warn("Telegram: no report file to attach")
